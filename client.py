...
class App(tk.Tk):
    ...
    def buy_products(self):
        ...
        # Iterate through the products in the basket
            ...
            messagebox.showwarning(title="Внимание!", message="Корзина пуста!")

    def return_product(self):
        print(">> Return products to table")

        # Get selected items from the basket table
        selected_items = self.basket_table.get_children()[-1]

        if not selected_items:
            # Show message to user about error
            messagebox.showerror("Ошибка возврата", "Вы не выбрали товары для возврата")
            return

        try:
            # Loop through selected items and add them back to the product table
            for item in selected_items:
                code_product = self.basket_table.item(item, "values")[0]
                user_count = int(self.basket_table.item(item, "values")[1])

                # Get current count of product in database
                db_count = int(self.get_count(id_product=code_product))

                # Calculate new count after returning items
                end_count = db_count + user_count

                # Update count in database
                self.update_data(code_product, end_count)

                # Remove item from basket table
                self.basket_table.delete(item)

            # Update total sum in basket
            self.sum = 0
            self.basket_sum_label.configure(text=f"Сумма: {self.sum}")

            # Show message to user about successful return
            messagebox.showinfo("Успешный возврат", "Товары успешно возвращены в таблицу")
